...
- `WeatherBot` class holds all logic:
  ...
  - `schedule_daily_message()` — converts the local `TIME_SEND_MESSAGE` to UTC and registers the
    daily `schedule` job; loops `run_pending()` every 60s.
...
## Conventions
- Times are stored/handled with `pytz`; the scheduler works in UTC internally but the message shows
  the local time and zone.
- Network failures degrade gracefully (return `None`, print, and still try to notify the chat of the
